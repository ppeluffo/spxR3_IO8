...
TaskHandle_t xHandle_idle, xHandle_tkCtl,xHandle_tkCmd, xHandle_tkData, xHandle_tkCounters, xHandle_tkGprsRx, xHandle_tkGprsTx, xHandle_tkOutputs, xHandle_tkXbee;
...
//------------------------------------------------------------------------------------
typedef enum { DEBUG_NONE = 0, DEBUG_GPRS, DEBUG_COUNT, DEBUG_XBEE } t_debug;
...
typedef struct {
	RtcTimeType_t rtc;
	float analog_data[NRO_ANALOG_CHANNELS];
	uint8_t digital_data[NRO_DIGITAL_CHANNELS];
